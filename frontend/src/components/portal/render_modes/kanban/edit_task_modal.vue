<template>
    <SharedModal title="Edit Task" @close="$emit('close')">
      <Form @submit="saveChanges" :validation-schema="schema" v-slot="{ errors }">
        <div class="form-group">
          <label for="title">Title</label>
          <Field name="title" v-model="editedTask.title" />
          <span class="error">{{ errors.title }}</span>
        </div>
        
        <div class="form-group">
          <label for="description">Description</label>
          <Field name="description" v-model="editedTask.description" as="textarea" />
          <span class="error">{{ errors.description }}</span>
        </div>
        
        <div class="form-group">
          <label>Subtasks</label>
          <div v-for="(subtask, index) in editedTask.subtasks" :key="index">
  ...
  export default {
    ...
    methods: {
      ...
      addSubtask() {
        this.editedTask.subtasks.push({ text: '', done: false });
      },
      removeSubtask(index) {
      ...
      addTag(newTag) {
        ...
        this.users.push(tag);
        this.editedTask.assignedTo.push(tag);
      },
